...
describe("MerkleTree", function () {
	...
	it("Insert two new leaves and verify the first leaf in an inclusion proof", async function () {
		...
		const secondLeafInput = {
			leaf: "2",
			path_elements: ["1", node9, node13],
			path_index: ["1", "0", "0"],
		};

		const { proof: secondProof, publicSignals: secondPublicSignals } =
			await groth16.fullProve(
				secondLeafInput,
				"circuits/circuit_js/circuit.wasm",
				"circuits/circuit_final.zkey"
			);
		const secondEditedPublicSignals =
			unstringifyBigInts(secondPublicSignals);
		const secondEditedProof = unstringifyBigInts(secondProof);
		const secondCalldata = await groth16.exportSolidityCallData(
			secondEditedProof,
			secondEditedPublicSignals
		);

		const secondArgv = secondCalldata
			.replace(/["[\]\s]/g, "")
			.split(",")
			.map((x) => BigInt(x).toString());

		const secondA = [secondArgv[0], secondArgv[1]];
		const secondB = [
			[secondArgv[2], secondArgv[3]],
			[secondArgv[4], secondArgv[5]],
		];
